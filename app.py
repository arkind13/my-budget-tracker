...
Can you please check this code and help





Can you please check this code and help in this error while hosting on streamlit cloud.
Traceback:
File "/mount/src/my-budget-tracker/app.py", line 153, in <module>
...
Reasoning
Diagnosis
The error occurs at line 153:


st.session_state.w_s = float(gs_data.get("Sunday Hours", 5.5))
The root cause is that gs_data (loaded from your Google Sheet via load_gsheet_data()) is returning a value for "Sunday Hours" that cannot be cast to float. Common culprits:

Scenario	What .get() returns	float() result
Cell is empty / NaN	nan (pandas)	✅ Works (returns nan)
Cell contains "" (empty string)	""	❌ ValueError
Cell contains text like "5 hrs"	"5 hrs"	❌ ValueError
Column has trailing space "Sunday Hours "	5.5 (default used)	✅ Works
Since the default 5.5 is only used when the key is missing, and the error is happening, it means the key exists but holds a non-numeric value (most likely an empty string "" or a string with text).

Fix
Replace the INITIALIZE SESSION STATE block with a safer version that coerces values properly:
...
def safe_float(value, default=0.0):
    ...
    try:
        result = float(value)
        # Reject NaN/inf — fall back to default
        if pd.isna(result) or not pd.notna(result):
            return default
        return result
...
if "initialized" not in st.session_state:
    gs_data = load_gsheet_data()
    st.session_state.pb_spent = safe_float(gs_data.get("Total Spent So Far"), 180.0)
    st.session_state.pb_adj   = safe_float(gs_data.get("Adjusted Amount"), 0.0)
    st.session_state.w_n      = safe_float(gs_data.get("Standard Hours"), 17.5)
    st.session_state.w_s      = safe_float(gs_data.get("Sunday Hours"), 5.5)
    st.session_state.w_l      = safe_float(gs_data.get("Late Night Hours"), 1.5)
    st.session_state.w_p      = safe_float(gs_data.get("Public Holiday Hours"), 0.0)
    st.session_state.initialized = True
What changed?
safe_float() helper — wraps float() in a try/except and also guards against NaN/inf using pd.isna(). If conversion fails for any reason, it returns the default value.

All six session state assignments now use safe_float() instead of bare float().

Additional Hardening (Recommended)
You should also fix load_gsheet_data() to strip whitespace from column names and handle empty cells before they reach your session state:
...
def load_gsheet_data():
    ...
    try:
        ...
        if not df.empty:
            ...
            # Get the first non-empty row as a dict
            row = df.iloc[0].to_dict()

            # Clean up: replace empty strings and NaN with None so .get() defaults work
            cleaned = {}
